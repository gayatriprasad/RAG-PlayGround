...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            name="run_eval",
            description=(
                "Run benchmark evaluation on an experiment and return summary scores. "
                "Runs the configured eval metrics on a subset of questions."
            ),
            inputSchema={
                "type": "object",
                "properties": {
                    "experiment": {
                        "type": "string",
                        "description": "Experiment name to evaluate",
                        "default": "01_format_comparison"
                    },
                    "max_questions": {
                        "type": "integer",
                        "description": "Maximum number of questions to evaluate",
                        "default": 10,
                        "minimum": 1,
                        "maximum": 500
                    }
...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    try:
        if name == "retrieve":
            ...
            return await _tool_run_eval(arguments)
        else:
...
async def _tool_run_eval(args: Dict) -> list[TextContent]:
    """Run benchmark evaluation and return summary scores."""
    experiment = args.get("experiment", "01_format_comparison")
    max_questions = args.get("max_questions", 10)

    logger.info(f"🧪 Running eval for: {experiment} (max_questions={max_questions})")
    ...
    try:
        ...
        from raglab.parsers.enterprise_bench import load_questions
        from raglab.eval.scorer import BenchmarkScorer

        questions = load_questions(cfg.benchmark)[:max_questions]

        results = []
        for q in questions:
            intent = classifier.classify(q.text)
        ...
        scored = scorer.score(results, cfg.eval)

        # Summarize
        correct = sum(1 for r in scored if r.answer_correct)
        avg_completeness = (
            sum(r.completeness for r in scored if r.completeness is not None)
            / max(1, sum(1 for r in scored if r.completeness is not None))
        )
        avg_overall = (
            sum(r.overall_score for r in scored if r.overall_score is not None)
            / max(1, sum(1 for r in scored if r.overall_score is not None))
        )

        summary = {
            "experiment": experiment,
            "questions_evaluated": len(scored),
            "correct": correct,
            "accuracy": round(correct / max(1, len(scored)), 3),
